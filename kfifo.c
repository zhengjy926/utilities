...
static void kfifo_copy_in(kfifo_t *fifo, const void *src, unsigned int len, size_t off);
static void kfifo_copy_out(kfifo_t *fifo, void *dst, unsigned int len, size_t off);
 ...
 */
unsigned int kfifo_out_linear(kfifo_t *fifo, unsigned int *tail, size_t n)
{
 ...
 * @return Available count till the end of data or till the end of the buffer
 */
unsigned int kfifo_out_linear_locked(kfifo_t *fifo, size_t *tail, unsigned int n)
{
    size_t ret;
    uint32_t primask = __get_PRIMASK();
    __disable_irq();
